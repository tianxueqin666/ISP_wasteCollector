...
# === Config ===
LOOKBACK = 60      # number of past days to look back
EPOCHS   = 60
BATCH    = 70
LR       = 5e-4
DROPOUT  = 0.2
USE_ALL_BINS = True  # combine all bins into one dataset
...
def make_sequences(x, y, lookback):
    """Create lookback-length sequences for LSTM input."""
    X_seq, y_seq = [], []
...
print(f"Total features: {len(feature_cols)} → {feature_cols[:10]} ...")
...
if USE_ALL_BINS:
    ...
    print(f"Using single bin {bin_id} with {len(g)} records.")
